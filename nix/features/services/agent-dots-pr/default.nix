{...}: {
  flake.modules.nixos."services-agent-dots-pr" = {
    ...
    secretsPresent =
      builtins.pathExists opencodeAuthSecret
      && builtins.pathExists ghTokenSecret;
  ...
  in
    lib.mkMerge [
      ...
      (lib.mkIf (!secretsPresent) {
        warnings = ["agent-dots-pr: secrets missing, service disabled"];
      })
    ];
}
